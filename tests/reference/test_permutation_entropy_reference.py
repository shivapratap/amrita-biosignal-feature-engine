...
@pytest.mark.parametrize("variant", ["original", "2x", "5x", "10x", "negative", "zscore"])
@pytest.mark.parametrize("order,delay", [(2, 1), (3, 1), (4, 1), (5, 2)])
def test_required_matrix_matches_antropy(
    ...
    actual = permutation_entropy(signal, order=order, delay=delay)
    expected_bits = antropy.perm_entropy(signal, order=order, delay=delay, normalize=False)
    assert actual == pytest.approx(expected_bits * np.log(2.0), abs=2e-15)


def test_known_antropy_fast_path_exception_is_not_adopted() -> None:
    """AntroPy's epsilon jitter reorders near-equal, but distinct, samples."""
    signal = required_signals()["periodic"]
    actual = permutation_entropy(signal, order=4, delay=2)
    expected_bits = antropy.perm_entropy(signal, order=4, delay=2, normalize=False)
    assert actual == pytest.approx(1.473881741077073, abs=2e-15)
    assert expected_bits * np.log(2.0) == pytest.approx(1.4610149834435189, abs=2e-15)
...
def test_randomized_normalized_matrix_matches_antropy(order: int, delay: int) -> None:
    ...
    actual = permutation_entropy(signal, order=order, delay=delay, normalize=True)
    expected = antropy.perm_entropy(signal, order=order, delay=delay, normalize=True)
